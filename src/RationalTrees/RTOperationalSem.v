...
Proof.
  constructor. apply initial_unification_state_wf.
  intros. apply first_nats_prop. apply HC. auto.
Qed.
...
Inductive label : Set :=
| Step : label
| Answer : unification_state -> label
.

Inductive eval_step : nt_state -> label -> state -> Prop :=
| esFail s : eval_step (Leaf Fail s) Step Stop
| esUnifyFail t1 t2 s : s == t1 |_| t2 => None -> eval_step (Leaf (Unify t1 t2) s) Step Stop
| esUnifySuccess t1 t2 s s' : s == t1 |_| t2 => (Some s')
                           -> eval_step (Leaf (Unify t1 t2) s) (Answer s') Stop
| esDisj g1 g2 s : eval_step (Leaf (Disj g1 g2) s) Step (NTState (Sum (Leaf g1 s) (Leaf g2 s)))
| esConj g1 g2 s : eval_step (Leaf (Conj g1 g2) s) Step (NTState (Prod (Leaf g1 s) g2))
| esFresh fg s n : eval_step (Leaf (Fresh fg) (s, n)) Step (NTState (Leaf (fg n) (s, S n)))
| esInvoke r arg s : eval_step (Leaf (Invoke r arg) s) Step (NTState (Leaf (proj1_sig (Prog r) arg) s))
...
| esProdAE nst g s : eval_step nst (Answer s) Stop
                  -> eval_step (Prod nst g) Step (NTState (Leaf g s))
| esProdSNE nst g nst' : eval_step nst Step (NTState nst')
                      -> eval_step (Prod nst g) Step (NTState (Prod nst' g))
| esProdANE nst g s nst' : eval_step nst (Answer s) (NTState nst')
                        -> eval_step (Prod nst g) Step (NTState (Sum (Leaf g s) (Prod nst' g)))
.
...
Lemma eval_step_unique nst l1 l2 st1 st2 (H1 : eval_step nst l1 st1) (H2 : eval_step nst l2 st2)
                     : l1 = l2 /\ st1 = st2.
Proof.
  generalize dependent st2. generalize dependent st1. revert l1 l2. induction nst; intros.
  * good_inversion H1; good_inversion H2; auto; constructor; auto.
    - eapply rt_unify_determined in H5; eauto. inversion H5.
    - eapply rt_unify_determined in H5; eauto. inversion H5.
    - eapply rt_unify_determined in H5; eauto. good_inversion H5. auto.
  * good_inversion H1; good_inversion H2; auto; eapply IHnst1 in H5; eauto;
      destruct H5 as [ H1 H2 ]; good_inversion H1; good_inversion H2; auto.
  * good_inversion H1; good_inversion H2; auto; eapply IHnst in H5; eauto;
      destruct H5 as [ H1 H2 ]; good_inversion H1; good_inversion H2; auto.
Qed.
...
| osNTState nst l st t : eval_step nst l st -> op_sem st t -> op_sem (NTState nst) (Cons l t)
.
...
Lemma op_sem_unique (st : state) (t1 t2 : trace) (H1 : op_sem st t1) (H2 : op_sem st t2)
                  : equal_streams t1 t2.
Proof.
  generalize dependent t2. generalize dependent t1. revert st.
  cofix CIH. intros. good_inversion H1; good_inversion H2. constructor.
  eapply eval_step_unique in H; eauto. destruct H. subst.
  constructor. auto. eapply CIH; eauto.
Qed.

(*
Definition in_denotational_analog (t : trace) (f : repr_fun) : Prop :=
  exists (s : subst) (n : nat), in_stream (Answer s n) t /\ [ s ,  f ].
