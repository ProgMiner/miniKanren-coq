...
(* repr funs *)
Definition repr_fun : Set := name -> ground_rt_term.

Definition repr_fun_eq (f1 f2 : repr_fun) : Prop := forall x, grt_eq (f1 x) (f2 x).

Lemma repr_fun_eq_refl (f : repr_fun) : repr_fun_eq f f.
Proof. intro. reflexivity. Qed.

Lemma repr_fun_eq_sym {f1 f2} (p : repr_fun_eq f1 f2) : repr_fun_eq f2 f1.
Proof. intro. symmetry. apply p. Qed.

Lemma repr_fun_eq_trans {f1 f2 f3} (p : repr_fun_eq f1 f2) (q : repr_fun_eq f2 f3)
                      : repr_fun_eq f1 f3.
Proof. intro. etransitivity. apply p. apply q. Qed.

Instance repr_fun_eq_equivalence : RelationClasses.Equivalence repr_fun_eq :=
  RelationClasses.Build_Equivalence _ repr_fun_eq_refl (fun _ _ => repr_fun_eq_sym)
    (fun _ _ _ => repr_fun_eq_trans).

Fixpoint apply_repr_fun (f : repr_fun) (t : term) : ground_rt_term :=
match t with
| Var n => f n
| Cst n => exist _ (CstInf n) (conj (CstInf_is_rt _) (CstInf_is_ground_inf_term n))
| Con n l r =>
  match apply_repr_fun f l, apply_repr_fun f r with
  | exist _ l (conj lrt lg), exist _ r (conj rrt rg) =>
    exist _ (ConInf n l r) (conj (ConInf_is_rt lrt rrt) (ConInf_is_ground_inf_term lg rg))
  end
end.

Lemma repr_fun_eq_apply (f1 f2 : repr_fun) (t : term) (FEQ : repr_fun_eq f1 f2)
                      : grt_eq (apply_repr_fun f1 t) (apply_repr_fun f2 t).
Proof.
  induction t; simpl; auto. reflexivity.
  destruct (apply_repr_fun f1 t1) as [ l1 [ l1rt l1g ] ].
  destruct (apply_repr_fun f1 t2) as [ r1 [ r1rt r1g ] ].
  destruct (apply_repr_fun f2 t1) as [ l2 [ l2rt l2g ] ].
  destruct (apply_repr_fun f2 t2) as [ r2 [ r2rt r2g ] ].
  unfold grt_eq in *. simpl in *. constructor; auto.
Qed.

Lemma apply_repr_fun_fv (f1 f2 : repr_fun) (t : term)
                        (H : forall x, (In x (fv_term t)) -> grt_eq (f1 x) (f2 x))
                        : grt_eq (apply_repr_fun f1 t) (apply_repr_fun f2 t).
Proof.
  induction t; simpl. apply H. constructor. auto. reflexivity.
  destruct (apply_repr_fun f1 t1) as [ l1 [ l1rt l1g ] ].
  destruct (apply_repr_fun f1 t2) as [ r1 [ r1rt r1g ] ].
  destruct (apply_repr_fun f2 t1) as [ l2 [ l2rt l2g ] ].
  destruct (apply_repr_fun f2 t2) as [ r2 [ r2rt r2g ] ].
  unfold grt_eq in *. simpl in *.
  constructor; [ apply IHt1 | apply IHt2 ]; intros; apply H;
    apply set_union_intro; [ left | right ]; auto.
Qed.
...
| dsgUnify f t1 t2 : grt_eq (apply_repr_fun f t1) (apply_repr_fun f t2) -> [| Unify t1 t2 , f |]
| dsgDisjL f g1 g2 : in_denotational_sem_goal g1 f -> [| Disj g1 g2 , f |]
| dsgDisjR f g1 g2 : in_denotational_sem_goal g2 f -> [| Disj g1 g2 , f |]
| dsgConj f g1 g2 : [| g1 , f |] -> [| g2 , f |] -> [| Conj g1 g2 , f |]
| dsgFresh f fn a fg : ~is_fv_of_goal a (Fresh fg) -> [| fg a , fn |]
                    -> (forall x, x <> a -> grt_eq (fn x) (f x)) -> [| Fresh fg , f |]
| dsgInvoke r t f : [| proj1_sig (Language.Prog r) t , f |] -> [| Invoke r t, f |]
...
| dslgUnify l f t1 t2 : grt_eq (apply_repr_fun f t1) (apply_repr_fun f t2)
                     -> [| S l | Unify t1 t2 , f |]
| dslgDisjL l f g1 g2 : [| l | g1 , f |] -> [| l | Disj g1 g2 , f |]
| dslgDisjR l f g1 g2 : [| l | g2 , f |] -> [| l | Disj g1 g2 , f |]
| dslgConj l f g1 g2 : [| l | g1 , f |] -> [| l | g2 , f |] -> [| l | Conj g1 g2 , f |]
| dslgFresh l f fn a fg : ~is_fv_of_goal a (Fresh fg) -> [| l | (fg a) , fn |]
                       -> (forall x, x <> a -> grt_eq (fn x) (f x)) -> [| l | Fresh fg , f |]
| dslgInvoke l r t f : [| l | (proj1_sig (Language.Prog r) t) , f |] -> [| S l | Invoke r t , f |]
where "[| n | g , f |]" := (in_denotational_sem_lev_goal n g f).

Hint Constructors in_denotational_sem_lev_goal : core.

Lemma in_denotational_sem_zero_lev g f : ~ [| 0 | g , f |].
Proof. intro. remember 0 as l. induction H; inversion Heql; auto. Qed.
...
Proof.
  ...
  { intros; destruct l'; auto; inversion LE. }
  { intros. destruct l'.
    { inversion LE. }
...
Proof.
  ...
  { constructor. apply IHDSG. intros.
    ...
    remember (Language.Prog r). destruct d as [rel [Hcl Hco]].
...
Proof.
  ...
  { induction g1; intros; good_inversion DSG1; good_inversion REN; good_inversion CG.
    ...
    { rename n into r. rename n0 into n.
      remember (Language.Prog r) as d. destruct d as [rel [Hcl Hco]].
